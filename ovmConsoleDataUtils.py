...
# Utils that do not need to access ovmConsoleData should go in ovmConsoleUtils,
# so that ovmConsoleData can use them without creating circular import problems
...
class MountVDI:
    def __init__(self, inVDI, inMode = None):
        ...
        try:
            ...
            self.mountPoint = tempfile.mkdtemp(".ovmconsole")
            ...
            if status != 0:
                try:
                    ...
                    ovmLogFailure('Device failed to unmount', e)
                output += '\n'+self.mountDev
...
class MountVDIDirectly:
    def __init__(self, inVDI, inMode = None):
        ...
        try:
            ...
            ovmLog('Mounted '+self.mountDev + ' on ' + self.mountPoint + ' mode ' + self.mode)
        except Exception, e:
            try:
                self.Unmount()
            except Exception, e:
    ...
    def Unmount(self):
        ...
        if self.mountedVDI:
            ...
            os.rmdir(self.mountPoint)
            self.mountedVDI = False
            ovmLog('Unmounted '+self.mountPoint)
        if status != 0:
            raise Exception(output)
