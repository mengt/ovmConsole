...
class LangErrors:
  ...
  errorMap = {
    ...
    'HOST_STILL_BOOTING' : 'OvmServer is still booting',
    ...
    'RESTORE_SCRIPT_FAILED' : 'The restore could not be performed because the restore script failed. Your OvmServer Backup file may be corrupt. See the logs for more information',
    ...
    'SESSION_INVALID' : 'UniCenter\'s connection to the server has been lost',
    ...
    'WLB_CONNECTION_REFUSED' : 'The WLB server refused a connection to OvmServer',
    ...
    'WLB_MALFORMED_REQUEST' : 'The WLB server rejected OvmServer\'s request as malformed',
    'WLB_MALFORMED_RESPONSE' : 'The WLB server said something that OvmServer wasn\'t expecting or didn\'t understand',
    ...
    'WLB_OVMSERVER_AUTHENTICATION_FAILED' : 'The WLB server reported that OvmServer rejected its configured authentication details',
    'WLB_OVMSERVER_CONNECTION_REFUSED' : 'The WLB server reported that OvmServer refused it a connection (even though we\'re connecting perfectly fine in the other direction)',
    'WLB_OVMSERVER_MALFORMED_RESPONSE' : 'The WLB server reported that OvmServer said something to it that WLB wasn\'t expecting or didn\'t understand',
    'WLB_OVMSERVER_TIMEOUT' : 'The WLB server reported that communication with OvmServer timed out',
    'WLB_OVMSERVER_UNKNOWN_HOST' : 'The WLB server reported that its configured server name for this OvmServer instance failed to resolve in DNS',
    'XAPI_HOOK_FAILED' : '3rd party xapi hook failed',
    'OVMAPI_MISSING_PLUGIN' : 'The requested plugin could not be found',
    'OVMAPI_PLUGIN_FAILURE' : 'There was a failure communicating with the plugin',
